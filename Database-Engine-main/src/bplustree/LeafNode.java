...
public class LeafNode extends Node implements Serializable{
	...
	public void delete(int index, String pageName) throws DBAppException {

		// Delete the specified page from the pageList if it exists
		if (!this.dictionary[index].pageList.remove(pageName)) {
			throw new DBAppException("The key does not reference this page");
		}
		
		// Delete dictionary pair from leaf if we removed the last page
		if(this.dictionary[index].pageList.size() == 0) {
			this.dictionary[index] = null;
			// Decrement numPairs
			numPairs--;
		}
	...
	public boolean insert(Key key, String pageName) {
	    // Perform binary search to find the key
	    int index = Arrays.binarySearch(dictionary, 0, numPairs, new DictionaryPair(key, ""));
	        
	    if (index >= 0) {
	        // Key found, add pageName to the page list
	        dictionary[index].pageList.add(pageName);
	        return true;
	    } else {
	        if (this.isFull()) {
	            /* Flow of execution goes here when numPairs == maxNumPairs */
	            ...
	            // Key not found, create new DictionaryPair and insert
	            index = -(index + 1); // Compute insertion point
	            // Shift elements to make space for newPair
	            System.arraycopy(dictionary, index, dictionary, index + 1, numPairs - index);
	            dictionary[index] = new DictionaryPair(key, pageName);
	            numPairs++;
	            return true;
	        }
	    }
	...
	public String toString() {
	    ...
	    for (int i = 0; i < numPairs; i++) {
	        ...
	        sb.append(dictionary[i].toString());
	    }
	    sb.append("]");
	    return sb.toString();
	}
